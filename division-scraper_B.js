    ...
    async function handleDateField(page, fieldSelector, targetYear, fieldType) {
		...
		try {
			...
			if (activeInterface.includes('date-picker') || activeInterface.includes('v-menu')) {
				// Set May 1, 2015 for start field
				if (fieldType === 'start') {
					await handleComplexDatePicker(page, 2015, activeInterface, 5, 1); // May 1
				} else if (fieldType === 'end') {
					// Set last day of Dec for end field
					const lastDayAug = 31; // December always has 31 days
					await handleComplexDatePicker(page, 2018, activeInterface, 8, 31); // December 31
				} else {
