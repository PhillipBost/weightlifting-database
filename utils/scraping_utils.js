...
async function getAthletesOnPage(athletesOnPage, page, filePath) {
    ...
    for (let i = 1; i <= athletesOnPage + 1; i++) {
        ...
        if (!isHeaderRow) {
            // DEBUG: Print first non-header row to debug column structure
            if (skippedCount === 0 && allAthleteData.length === 0) {
                console.log('DEBUG SCRAPING UTILS RAW ROW:', JSON.stringify(athleteData));
            }
