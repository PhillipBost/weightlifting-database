...
exit`;
            ...
            const batchFile = path.join(process.cwd(), 'run_upload_2015.bat');
...
async function scrapeAthleteProfileIntegrated(page, athleteName, ageCategory, weightClass, competitionDate, divisionIndex = 0, issuesLogger = null, divisionNumber = 0, divisionName = '') {
    ...
    async function handleDateField(page, fieldSelector, targetYear, fieldType) {
		...
		try {
			...
			if (activeInterface.includes('date-picker') || activeInterface.includes('v-menu')) {
				// Set Jan 1, 2024 for start field
				if (fieldType === 'start') {
    ...
    try {
        ...
        await page.click('#maximum_lifter_age');
        await page.keyboard.down('Control');
        await page.keyboard.press('a');
        await page.keyboard.up('Control');
        
        await page.type('#maximum_lifter_age', `${maximumAge}`, {delay: 2});
        await page.waitForTimeout(500);
        await page.keyboard.press('ArrowDown');
		...
		console.log(`📅 Setting START date range to 2024...`);
		await handleDateField(page, '#form__date_range_start', 2015, 'start');
		await page.waitForTimeout(100);

		console.log(`📅 Setting END date range to 2024...`);
		await handleDateField(page, '#form__date_range_end', 2015, 'end');
		await page.waitForTimeout(100);

		console.log(`Date range set to: 01-01-2024 - 12-31-2024`);
