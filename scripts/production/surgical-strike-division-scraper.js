...
function smartSortDivisions(divisions, result) {
    ...
    // Prioritize: exact match > inactive exact match > others
    const exactMatch = divisionEntries.filter(([name]) => name === expectedDivision);
    const inactiveMatch = divisionEntries.filter(([name]) => name === expectedInactive);
