        ...
        if (targetAthlete) {
            ...
            if (targetAthlete.internalId) {
                ...
                if (!nullError && nullInternalIdCandidates && nullInternalIdCandidates.length >= 1) {
                    ...
                    if (targetAthlete.membershipNumber) {
                        // IDENTITY GUARD: Before updating the lifter with a newly found membership number/internal ID,
                        // check if another record already owns this internal_id.
                        if (targetAthlete.internalId) {
                            const { data: masterCheck } = await supabase
                                .from('usaw_lifters')
                                .select('lifter_id')
                                .eq('internal_id', targetAthlete.internalId)
                                .neq('lifter_id', lifterId)
                                .single();

                            if (masterCheck) {
                                console.log(`    🚨 Identity Collision: Internal ID ${targetAthlete.internalId} already belongs to Master Lifter ${masterCheck.lifter_id}.`);
                                console.log(`    ♻️ Resolution: Redirecting results to Master ${masterCheck.lifter_id} instead of updating Duplicate ${lifterId}.`);
                                return {
                                    lifterId: masterCheck.lifter_id,
                                    scrapedData: targetAthlete
                                };
                            }
                        }
        ...
        try {
            ...
            if (lifter?.internal_id) {
                ...
                if (foundInternalId) {
                    ...
                    if (result.verified) {
                        // IDENTITY GUARD: Before adding internal_id to this record, check if a Master already exists
                        const { data: masterCheck } = await supabase
                            .from('usaw_lifters')
                            .select('lifter_id')
                        ...
                        if (masterCheck) {
                            console.log(`    🚨 Identity Collision: Internal ID ${foundInternalId} already belongs to Master Lifter ${masterCheck.lifter_id}.`);
                            console.log(`    ♻️ Resolution: Redirecting to Master ${masterCheck.lifter_id} instead of enriching Duplicate ${lifterId}.`);
                            return {
                                lifterId: masterCheck.lifter_id,
                                metadata: result.metadata
                            };
