...
async function getWsoStatistics() {
    ...
    const [totalResults, withWso, withoutWso] = await Promise.all([
        supabase.from('meet_results').select('result_id', { count: 'exact', head: true }),
        supabase.from('meet_results').select('result_id', { count: 'exact', head: true }).not('wso', 'is', null),
        supabase.from('meet_results').select('result_id', { count: 'exact', head: true }).is('wso', null)
    ]);
