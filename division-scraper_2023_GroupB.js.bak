...
function loadDivisions() {
    const divisionsFile = './all divisions_GroupB_2022.csv';
...
exit`;
            ...
            const batchFile = path.join(process.cwd(), 'run_upload_2022.bat');
    ...
    async function handleDateField(page, fieldSelector, targetYear, fieldType) {
		...
		try {
			...
			if (activeInterface.includes('date-picker') || activeInterface.includes('v-menu')) {
				...
				if (fieldType === 'start') {
					await handleComplexDatePicker(page, 2022, activeInterface, 1, 1); // Jan 1
				} else if (fieldType === 'end') {
					// Set last day of Dec for end field
					const lastDayDec = 31; // December always has 31 days
					await handleComplexDatePicker(page, 2022, activeInterface, 12, lastDayDec); // December 31
				} else {
...
async function processAllDivisions() {
    ...
    const completedDivisionsLog = './completed_divisionsB.csv';
    const logHeaders = [
        'division_number',
        'division_name',
        'timestamp',
