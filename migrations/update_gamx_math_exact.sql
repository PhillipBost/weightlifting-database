-- Migration: Upgrade Math Functions to Exact Precision (Series/CF)
...
-- Iterate until term is negligible (or max iterations)
sum := x;
-- First term (n=0): x / 1
FOR n IN 1..60 LOOP term := - term * x2 * (2 * n - 1) / (n * (2 * n + 1));
sum := sum + term;
EXIT
WHEN abs(term) < '1e-20'::NUMERIC;
END LOOP;
RETURN coef * sum;
END;
...
-- 1 / (x + (1/2) / (x + (1) / (x + (3/2) / ... )))
-- Evaluated backwards for stability
val := 0;
-- 60 iterations provides robust precision for x >= 1
FOR n IN REVERSE 60..1 LOOP val := (n * 0.5) / (x + val);
END LOOP;
...
CREATE OR REPLACE FUNCTION gamx_erf(x NUMERIC) RETURNS NUMERIC AS $$ BEGIN IF abs(x) < 1.0 THEN RETURN gamx_erf_series(x);
ELSE IF x >= 0 THEN RETURN 1.0 - gamx_erfc_cf(x);
ELSE -- erf(-x) = -erf(x)
RETURN -(1.0 - gamx_erfc_cf(abs(x)));
END IF;
END IF;
END;
$$ LANGUAGE plpgsql IMMUTABLE;
