-- Fix: update_qpoints_on_change should use existing competition_age column
-- Bug: Function was calculating competition_age from date/birth_year and ignoring
--      the existing NEW.competition_age value, causing NULL age when birth_year is NULL
-- Impact: Q-scores not calculated when competition_age is set by surgical strike scraper
...
BEGIN
    -- FIXED: Use existing competition_age from column if available, otherwise calculate
    IF NEW.competition_age IS NOT NULL THEN
        competition_age := NEW.competition_age;
    ELSIF NEW.date IS NOT NULL AND NEW.birth_year IS NOT NULL THEN
       ...
       (OLD.q_masters IS NULL AND public.is_master_age(NEW.gender, competition_age)) THEN
        ...
        IF competition_age IS NOT NULL AND competition_age BETWEEN 10 AND 20 AND
           lifter_gender IS NOT NULL AND bodyweight_numeric > 0 AND total_numeric > 0 THEN
            youth_age_factor := get_youth_age_factor_interpolated(competition_age, bodyweight_numeric, lifter_gender);
