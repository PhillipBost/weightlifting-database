...
async function scrapeAthleteProfileIntegrated(page, athleteName, ageCategory, weightClass, competitionDate, divisionIndex = 0) {
    ...
    try {
        ...
        while (hasMorePages) {
            ...
            const pageAthletes = await page.evaluate(() => {
                ...
                rows.forEach(row => {
                    ...
                    if (cellTexts.length >= 8 && cellTexts[7]) {
						console.log('TABLE ROW DATA:', cellTexts.slice(0, 10)); // Show first 10 columns
						athletes.push({
            ...
            console.log(`👥 Found ${pageAthletes.length} athletes on page ${currentPage}`);
        ...
        console.log(`🎯 Total athletes found in division: ${allDivisionAthletes.length}`);
