...
async function scrapeAthleteProfileIntegrated(page, athleteName, ageCategory, weightClass, competitionDate, divisionIndex = 0, issuesLogger = null, divisionNumber = 0, divisionName = '') {
    ...
    async function handleDateField(page, fieldSelector, targetYear, fieldType) {
		...
		try {
			...
			if (activeInterface.includes('date-picker') || activeInterface.includes('v-menu')) {
				// Set Jan 1, 2014 for start field
				if (fieldType === 'start') {
					...
					await handleComplexDatePicker(page, 2014, activeInterface, 12, 31); 
				} else {
					await handleComplexDatePicker(page, 2014, activeInterface); // Default for other
				}
    ...
    try {
		...
		console.log(`📅 Setting START date range to 2014...`);
		await handleDateField(page, '#form__date_range_start', 2014, 'start');
		await page.waitForTimeout(100);

		console.log(`📅 Setting END date range to 2014...`);
		await handleDateField(page, '#form__date_range_end', 2014, 'end');
		await page.waitForTimeout(100);

		console.log(`Date range set to: 01-01-2014 - 12-31-2014`);
