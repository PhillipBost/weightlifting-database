    ...
    async function handleDateField(page, fieldSelector, targetYear, fieldType) {
		...
		try {
			...
			if (activeInterface.includes('date-picker') || activeInterface.includes('v-menu')) {
				// Set Jan 1, 2013 for start field
				if (fieldType === 'start') {
					...
					await handleComplexDatePicker(page, 2013, activeInterface, 12, 31); 
				} else {
					await handleComplexDatePicker(page, 2013, activeInterface); // Default for other
				}
