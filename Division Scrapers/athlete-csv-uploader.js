...
async function updateMeetResultsWithWSO(athleteData, lifterId, errorLogger) {
    if (!athleteData.lift_date || !athleteData.wso || !athleteData.club_name) {
        console.log(`  ⏭️ Skipping meet_results update - missing lift_date, wso, or club_name`);
        return { updated: 0, errors: 0 };
    }

    try {
        console.log(`  🔍 Looking for meet_results: lifter_id=${lifterId}, date=${athleteData.lift_date}`);
        
        // Find matching meet_results records
        const { data: meetResults, error: findError } = await supabase
            .from('meet_results')
            .select('result_id, meet_name, wso, club_name')
            .eq('lifter_id', lifterId)
            .eq('date', athleteData.lift_date);
        ...
        if (!meetResults || meetResults.length === 0) {
            console.log(`  📝 No meet_results found for ${athleteData.athlete_name} on ${athleteData.lift_date}`);
            return { updated: 0, errors: 0 };
        }
        
        console.log(`  📊 Found ${meetResults.length} meet_results records to update`);
        ...
        console.log(`  ✅ Updated ${meetResults.length} meet_results records with WSO/club info`);
        meetResults.forEach(result => {
            console.log(`    - Result ID ${result.result_id}: ${result.meet_name || 'Unknown Meet'}`);
            console.log(`      WSO: "${result.wso}" → "${athleteData.wso}"`);
            console.log(`      Club: "${result.club_name}" → "${athleteData.club_name}"`);
        });
        
        return { updated: meetResults.length, errors: 0 };
        
    } catch (error) {
        console.error(`  💥 Error updating meet_results for ${athleteData.athlete_name}:`, error.message);
    ...
    try {
        ...
        for (const athleteData of athleteRecords) {
            ...
            try {
                ...
                const liftDate = athleteData.lift_date;
                
                // Primary lookup: by athlete_name (most reliable)
                ...
                if (matchingLifters.length > 1) {
                    errorLogger.logError(
                        ...
                        athleteData.membership_number, 
                        'DUPLICATE_NAMES', 
                        `Found ${matchingLifters.length} lifters with name "${athleteName}"`
                    );
                    duplicateCount++;
                    continue;
                }
                ...
                console.log(`  🔍 Found lifter: ${lifter.athlete_name} (lifter_id: ${lifterId})`);
                
                // Prepare lifter table update
                const updateData = {
                    lifter_id: lifterId,
                    membership_number: athleteData.membership_number ? parseInt(athleteData.membership_number) : null,
                    gender: athleteData.gender?.toString().trim() || null,
                ...
                // Update meet_results table with temporal WSO/club data using lifter_id + date
                const meetResultsUpdate = await updateMeetResultsWithWSO(athleteData, lifterId, errorLogger);
        ...
        if (newLifters.length > 0) {
            ...
            for (let i = 0; i < newLifters.length; i++) {
                ...
                try {
                    ...
                    // Also try to update meet_results for the new lifter
                    const meetResultsUpdate = await updateMeetResultsWithWSO(athleteData, newLifterId, errorLogger);
                    meetResultsUpdated += meetResultsUpdate.updated;
                    errorCount += meetResultsUpdate.errors;
        ...
        console.log(`  📊 Results: ${lifterUpdates.length} lifters updated, ${newLiftersCreated} created, ${meetResultsUpdated} meet_results updated, ${duplicateCount} duplicates, ${errorCount} errors`);
        
        return { 
            liftersUpdated: lifterUpdates.length, 
            liftersCreated: newLiftersCreated,
...
async function main() {
    console.log('🏋️ Athlete CSV to Supabase Upload Started (Dual Table Update)');
    console.log('================================================================');
    ...
    try {
        ...
        let totalLiftersUpdated = 0;
        let totalLiftersCreated = 0;
        ...
        for (const filePath of csvFiles) {
            ...
            try {
                ...
                totalLiftersUpdated += result.liftersUpdated;
                totalLiftersCreated += result.liftersCreated || 0;
        ...
        // Report results
        console.log('\n📊 Dual Table Update Summary:');
        console.log(`📁 CSV files processed: ${filesProcessed}`);
        console.log(`✅ Lifters table updated: ${totalLiftersUpdated}`);
        console.log(`➕ New lifters created: ${totalLiftersCreated}`);
