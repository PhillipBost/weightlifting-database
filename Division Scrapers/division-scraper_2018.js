...
function loadDivisions() {
    const divisionsFile = './all divisions_2018-2019.csv';
...
exit`;
            ...
            const batchFile = path.join(process.cwd(), 'run_upload_2018-2019.bat');
    ...
    async function handleDateField(page, fieldSelector, targetYear, fieldType) {
		...
		try {
			...
			if (activeInterface.includes('date-picker') || activeInterface.includes('v-menu')) {
				...
				if (fieldType === 'start') {
					await handleComplexDatePicker(page, 2018, activeInterface, 1, 1); // Jan 1
				} else if (fieldType === 'end') {
					// Set last day of Dec for end field
					const lastDayDec = 31; // December always has 31 days
					await handleComplexDatePicker(page, 2019, activeInterface, 12, lastDayDec); // December 31
				} else {
